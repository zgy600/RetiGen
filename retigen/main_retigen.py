...
from target import train_target_domain_RetiGenBench as train
...
# run
@hydra.main(config_path="configs", config_name="root", version_base=None)
def main(args):
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.train_source:
        ...
        src_domain = args.data.source_domains
        args.data.src_domain = src_domain
        # tgt_domain = args.data.target_domains
        for tgt_domain in args.data.target_domains:
            ...
            if use_wandb(args):
                ...
                wandb.init(
                    project=args.project if args.project else args.data.dataset,
                    group=args.memo,
                    name=f"{args.name}" if args.name else f"seed_{args.seed}",
                    # name=f"{args.name}_seed_{args.seed}" if args.name else f"seed_{args.seed}",
                    config=dict(args),
                )
            # main loop
