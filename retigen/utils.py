...
NUM_CLASSES = {"domainnet-126": 126, "VISDA-C": 12, "GDRBench": 5}
...
@torch.no_grad()
def concat_all_gather_tuple(tuple_data):
    """
    Performs all_gather operation on the provided tuple.
    *** Warning ***: torch.distributed.all_gather has no gradient.
    """
    # 创建一个空的列表，用于存储每个进程的数据
    gathered_data = []

    # 执行分布式数据收集操作
    for data_item in tuple_data:
        # 创建一个与当前数据项相同类型和结构的数据项
        gathered_data_item = type(data_item)()

        # 执行 all_gather 操作
        dist.all_gather([gathered_data_item], data_item)

        # 将收集到的数据项添加到列表中
        gathered_data.append(gathered_data_item)
