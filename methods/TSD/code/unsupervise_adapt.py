...
from alg.algs.GDRNet import GDRNet
from alg import alg
...
def adapt_loader(args):
    ...
    testset = ImageFolder(root=data_root,transform=test_transform, target_transform=target_transform)
...
if __name__ == '__main__':    
    ...
    algorithm.renew_model(pretrain_model_path)
    ...
    for idx,sample in enumerate(dataloader):
        ...
        outputs_arr.append(output_sf.detach().cpu())
        labels_arr.append(label)
        
        
    outputs_arr = torch.cat(outputs_arr,0).numpy()
    labels_arr = torch.cat(labels_arr).numpy()
    pred_arr = outputs_arr.argmax(1)
    
    acc = accuracy_score(labels_arr, pred_arr)
    f1 = f1_score(labels_arr, pred_arr, average='macro')
    auc_ovo = roc_auc_score(labels_arr, outputs_arr, average='macro', multi_class='ovo')
    matrix = confusion_matrix(labels_arr, pred_arr)
    print(matrix)
